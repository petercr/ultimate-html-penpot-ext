...
### Importer visual baseline fixtures
...
- Enforces one wall-clock budget of 15 seconds per chain and streams responses with hard caps of 3 MB for pages and trusted-mode assets, 2 MB for CSS/SVG assets, and 1.5 MB for fonts, counted after decompression and independent of declared `Content-Length`.
- Returns HTML/XHTML for pages, SVG for `mode=svg`, CSS for `mode=css`, inert font bytes for `mode=font`, and upstream-typed bytes for trusted-mode `mode=asset` requests, plus the final validated upstream URL in `X-HTML-Source-URL` so relative assets resolve correctly.
...
- Direct page URL imports also use credential-free browser requests. When they are blocked by CORS **and** the hosted fetch service is configured, the requested page or explicitly requested asset URL is sent to this project's service so it can fetch that resource server-side within the mode-specific limits described above and return the bytes to the plugin. It does not receive user identity beyond IP-derived rate limiting (stored only as a hash), does not retain content, and never forwards credentials. Pasted HTML is never sent through the service.
- Remote SVG inlining, webfont inlining, and trusted-mode same-origin page requests use the same constrained service path (`mode=svg`, `mode=font`/`mode=css`, and `mode=asset`) when direct fetching fails or browser CORS would block the capture.
